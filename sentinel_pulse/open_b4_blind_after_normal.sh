#!/usr/bin/env bash
# Open B4 blind evaluation only after the exact independently validated normal run passes.
set -Eeuo pipefail

CONTROL_ROOT=${CONTROL_ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)}
RUNTIME_ROOT=${RUNTIME_ROOT:-/home/dat/eBPF-project-runtime-pulse-b4}
NORMAL_EVIDENCE_ROOT=${NORMAL_EVIDENCE_ROOT:?point to terminal B4 normal evidence}
MODEL_SOURCE=${MODEL_SOURCE:-$RUNTIME_ROOT/.runtime-artifacts/sentinel-pulse-a2-b4-model}
POLICY_SOURCE=${POLICY_SOURCE:-$RUNTIME_ROOT/sentinel_pulse/protocol/decision-policy-temporal-b4.json}
ATTACK_CONTRACT=${ATTACK_CONTRACT:-$CONTROL_ROOT/sentinel_pulse/protocol/blind-attack-contract-b4.json}
...
RUN_ID=${RUN_ID:-sentinel-pulse-formal-blind-b4-$(date -u +%Y%m%dT%H%M%SZ)}
EVIDENCE_ROOT=${EVIDENCE_ROOT:-/home/dat/sentinel-pulse-evidence/blind-b4/$RUN_ID}
