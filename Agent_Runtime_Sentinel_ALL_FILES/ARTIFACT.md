...
| Generalization V9 protocol | `ml-service/generalization_evaluation_contract.json`, `validate_generalization_contract.py`, `workload_version_identity.py` | pre-evidence LOWO/version-shift/cross-cluster contract plus privacy-safe immutable Pod/image/version resolver; requires new seeds, no held-out calibration/refit and hash-bound V8 parents |
