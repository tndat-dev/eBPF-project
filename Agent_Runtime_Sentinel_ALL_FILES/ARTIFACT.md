...
| Falco blind-attack finalizer | `falco_attack_evidence_finalizer.py`, integrated by `run_v8_blind_attack.sh` | maps privacy-safe Falco decisions to 200 same-pod horizons, right-censors at the next injection with a 1 s pre-ack guard, reports Wilson recall CI/latency and gates the marker on fresh six-reader coverage |
...
| V8 canonical attack replay | `run_aims_blind_matrix.py`, `merge_feature_captures.py`, `build_feature_replay_dataset.py` | validates and merges every child capture, requires 200 injection intervals, labels only exact same-pod/release/run/phase/regime intersections and records labels_used_for_training=false |
...
| Shared-workload routing ablation | `ml-service/train_shared_workload_candidate.py`, `SharedWorkloadModelManager` | fits exactly one pooled model from run-01 only, preserves workload-specific behavior limits, creates independent fit-only calibration and replays unchanged run-02--06 holdouts; a development-gate rejection may be replayed only as explicitly marked evaluation-only evidence and can never be promoted |
...
| Tetragon paired rule baseline | `ml-service/evaluate_tetragon_rule_replay.py` | replays the frozen sensitive-syscall rule on the same 20 normal phases/200 attacks, binds attack attribution to exact run/phase identity and emits hash-checked alert-rate/Wilson CI evidence |
