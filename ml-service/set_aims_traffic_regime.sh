...
case "$REGIME" in
  steady)   base_replicas=1; mix_replicas=0; sleep_seconds=1 ;;
  burst)    base_replicas=6; mix_replicas=2; sleep_seconds=0 ;;
  recovery) base_replicas=1; mix_replicas=0; sleep_seconds=2 ;;
  toolmix)  base_replicas=2; mix_replicas=4; sleep_seconds=1 ;;
  idle)     base_replicas=0; mix_replicas=0; sleep_seconds=2 ;;
...
printf 'AIMS traffic regime=%s base_replicas=%d readmix_replicas=%d sleep=%ss\n' \
  "$REGIME" "$base_replicas" "$mix_replicas" "$sleep_seconds"
