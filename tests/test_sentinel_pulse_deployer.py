...
class PulseDeployerTests(unittest.TestCase):
    ...
    def test_candidate_detector_is_checksum_gated_unprivileged_and_audit_only(self):
        ...
        self.assertIn("if [[ $REQUIRE_CONTROL_COLLECTOR == true ]]", installer)
        self.assertIn("Wants=sentinel-pulse-collector\\.service", installer)
        self.assertIn("render an isolated variant", installer)
    ...
    def test_b5_blind_opener_binds_normal_model_policy_and_runtime(self):
        script = (
            ROOT / "sentinel_pulse" / "open_b5_blind_after_normal.sh"
        ).read_text()
        self.assertIn('test -f "$NORMAL_EVIDENCE_ROOT/NORMAL_PASS"', script)
        self.assertIn('test ! -e "$NORMAL_EVIDENCE_ROOT/FAILED"', script)
        self.assertIn("candidate_binding.runtime_source_git_commit", script)
        self.assertIn("candidate_binding.model_manifest_sha256", script)
        self.assertIn("candidate_binding.decision_policy_sha256", script)
        self.assertIn("blind-attack-contract-b5.json", script)

    def test_b6_blind_opener_binds_normal_model_policy_and_runtime(self):
        script = (
            ROOT / "sentinel_pulse" / "open_b6_blind_after_normal.sh"
        ).read_text()
        self.assertIn('test -f "$NORMAL_EVIDENCE_ROOT/NORMAL_PASS"', script)
        self.assertIn('test ! -e "$NORMAL_EVIDENCE_ROOT/FAILED"', script)
        self.assertIn("candidate_binding.runtime_source_git_commit", script)
        self.assertIn("candidate_binding.model_manifest_sha256", script)
        self.assertIn("candidate_binding.decision_policy_sha256", script)
        self.assertIn("blind-attack-contract-b6.json", script)

    def test_b7_blind_opener_binds_normal_model_policy_and_runtime(self):
        script = (
            ROOT / "sentinel_pulse" / "open_b7_blind_after_normal.sh"
        ).read_text()
        self.assertIn('test -f "$NORMAL_EVIDENCE_ROOT/NORMAL_PASS"', script)
        self.assertIn('test ! -e "$NORMAL_EVIDENCE_ROOT/FAILED"', script)
        self.assertIn("candidate_binding.runtime_source_git_commit", script)
        self.assertIn("candidate_binding.model_manifest_sha256", script)
        self.assertIn("candidate_binding.decision_policy_sha256", script)
        self.assertIn("blind-attack-contract-b7.json", script)
        self.assertIn("eBPF-project-runtime-pulse-b7", script)
    ...
    def test_formal_soak_requires_pressure_free_stable_cluster(self):
        ...
        self.assertIn('DECISION_POLICY_SOURCE="$REMOTE_ROOT/$policy_rel"', starter)
        self.assertIn("installed_policy_sha", starter)
        self.assertIn('[[ $installed_policy_sha == "$policy_sha" ]]', starter)
        self.assertIn("trap interrupt INT TERM", starter)
